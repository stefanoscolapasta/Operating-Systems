...
{ 
	...
	if (sync_count < SYNC_MAX) {
		...
		if( rc ) PrintERROR_andExit(rc, "pthread_cond_broadcast failed"); /* no EINTR */

		/* aggiunta SOLO PER VERIFICARE che la broadcast
		    sblocca solo quelli gia' bloccati 
		
		   printf("Sono %lu e faccio la cond_wait #2 ....."
			" mi sblocchero' ?  TEMO DI NO ....\n", pthread_self() );
		   rc = pthread_cond_wait(&sync_cond, &sync_lock);
		   if( rc ) PrintERROR_andExit(rc, "pthread_cond_wait #2 failed");
		   printf("Sono %lu e mi sono sbloccato, "
			" ora faccio la unlock\n", pthread_self() );
		*/		 
	}
